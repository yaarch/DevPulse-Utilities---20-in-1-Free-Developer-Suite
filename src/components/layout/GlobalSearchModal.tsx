...
export const GlobalSearchModal: React.FC = () => {
  const { isSearchOpen, setIsSearchOpen, navigateToTool, t, recentTools } = useApp();
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-150">
      ...
      >
          ...
          {displayTools.length > 0 ? (
            <div>
              <div className="px-3 py-1.5 text-xs font-semibold text-slate-400 flex items-center gap-1.5">
                ...
                <span>{query ? `Results (${displayTools.length})` : 'Recently Used & Suggested'}</span>
              </div>
        ...
        <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <div className="flex items-center gap-3">
            <span>
              Use <kbd className="px-1 py-0.5 bg-white dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-700">↑</kbd> <kbd className="px-1 py-0.5 bg-white dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-700">↓</kbd> to navigate
            </span>
            <span>
              <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-700">↵</kbd> to select
            </span>
          </div>
          <span>Total: {TOOLS_LIST.length} tools</span>
        </div>
      </div>
    </div>
  );
